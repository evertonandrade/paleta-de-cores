...
function hslControls(event) {
  const dataBright = event.target.getAttribute("data-bright");
  const dataSat = event.target.getAttribute("data-sat");
  const dataHue = event.target.getAttribute("data-hue");
  const index = dataBright || dataSat || dataHue;
...
function copyToClipboard(hex) {
  ...
  elemet.value = hex.innerText;
  document.body.appendChild(elemet);
  elemet.select();
  document.execCommand("copy");
  document.body.removeChild(elemet);
...
function savePalette(event) {
  ...
  deletePaletteBtn.addEventListener("click", (event) => {
    const customPallete = event.target.parentElement;
    customPallete.classList.add("fall");
    ...
    customPallete.addEventListener("transitionend", function () {
      customPallete.remove();
    });
  });
